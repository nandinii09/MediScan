...
# Load known faces and their medical records
...
def load_known_faces():
    ...
    for row in rows:
        face_image_path = f"faces/{row['uniqueid']}.jpg"  # Assuming you save face images with unique ID as filename
        face_image = face_recognition.load_image_file(face_image_path)
        face_encoding = face_recognition.face_encodings(face_image)[0]
        known_face_encodings.append(face_encoding)
        known_face_ids.append(row['uniqueid'])
        known_face_records.append(row)
...
def recognize_face(frame):
    ...
    for face_encoding in face_encodings:
        ...
        if matches[best_match_index]:
            uniqueid = known_face_ids[best_match_index]
            record = known_face_records[best_match_index]
            return uniqueid, record
...
def process_frame(frame):
    ...
    # Recognize face and fetch previous medical records
    uniqueid, record = recognize_face(frame)
    if uniqueid:
        cv2.putText(frame, f"ID: {uniqueid}", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv2.putText(frame, f"Name: {record['name']}", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv2.putText(frame, f"Age: {record['age']}", (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv2.putText(frame, f"Previous Height: {record['height']} cm", (10, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv2.putText(frame, f"Previous Pulse: {record['pulse']} bpm", (10, 240), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv2.putText(frame, f"Previous Temp: {record['temperature']} °C", (10, 270), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
...
def save_record():
    ...
    cursor = conn.cursor()
    cursor.execute('''
    INSERT INTO patients (uniqueid, name, age, height, pulse, temperature, timestamp, day)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ''', (uniqueid, name, age, height, pulse, temperature, timestamp, day))
